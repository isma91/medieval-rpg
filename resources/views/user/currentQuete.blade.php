...
<div class="container">
    ...
    if (Session::has('messageBonneReponse')) {
        ?>
        <div class="alert alert-success" role="alert">{!! Session::get('messageBonneReponse') !!}</div>
        <?php
    }
    ...
    if (Session::has('messageLoose')) {
        ?>
        <div class="alert alert-warning" role="alert">{!! Session::get('messageLoose') !!}</div>
        <?php
    }
    if (Session::has('messageWin')) {
        ?>
        <div class="alert alert-success" role="alert">{!! Session::get('messageWin') !!}</div>
        <?php
    }
    ...
    <div class="row">
        <div class="col-lg-12">
            <h1 class="page-header">Bienvenue Player {!! Auth::user()->name !!} {!! Auth::user()->lastname !!}
                <small>Level {!! Auth::user()->level !!}</small>
            </h1>
            <h2><b><?php echo $currentQuete[0]->name; ?></b></h2>
            {!! Form::open(['url' => 'goEtape']) !!}
            <div class="form-group">
                {!! Form::label('personnage', 'Choisissez votre personnage :') !!}
            ...
            <div class="form-group">
                {!! Form::label('type', 'Choisissez le type d étape :') !!}
